var australiaDownloadTableSenna = document.getElementById("australia-download-table-senna");
var singapuraDownloadTableSenna = document.getElementById("singapura-download-table-senna");
var brasilDownloadTableSenna = document.getElementById("brasil-download-table-senna");
var miamiDownloadTableSenna = document.getElementById("miami-download-table-senna");
var monzaDownloadTableSenna = document.getElementById("monza-download-table-senna");
var silverstoneDownloadTableSenna = document.getElementById("silverstone-download-table-senna");
var euaDownloadTableSenna = document.getElementById("eua-download-table-senna");
var historicoAustraliaSenna = document.getElementById("australia-historico-senna");


australiaDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa1.png'

})
monzaDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa2.png'

})
singapuraDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa3.png'

})
brasilDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa4.png'

})
miamiDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa5.png'

})
silverstoneDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa6.png'

})
euaDownloadTableSenna.addEventListener("click",function(){
    window.location = '../tabelas/sennat8-etapa7.png'

})

historicoAustraliaSenna.addEventListener("click",function(){
    window.location = '../senna/australia.html'
})
